...
void NF_RunTime_ISR_DataBuffer_Add(CLR_UINT8 *dataBuffer, CLR_UINT8 *data)
{
    ...
    dataBuffer += sizeof(NF_Runtime_ISR_MemoryOffsetType);
    NF_Runtime_ISR_MemoryOffsetType *count = (NF_Runtime_ISR_MemoryOffsetType *)dataBuffer;
    dataBuffer += sizeof(NF_Runtime_ISR_MemoryOffsetType);
...
bool NF_RunTime_ISR_DataRingBuffer_Push(CLR_UINT8 *dataBuffer, CLR_UINT8 *data, bool allowOverwrite)
{
    ...
    NF_Runtime_ISR_MemoryOffsetType elementSize = *(NF_Runtime_ISR_MemoryOffsetType *)dataBuffer;
    dataBuffer += sizeof(NF_Runtime_ISR_MemoryOffsetType);
    NF_Runtime_ISR_MemoryOffsetType *indexToWriteData = (NF_Runtime_ISR_MemoryOffsetType *)dataBuffer;
    dataBuffer += sizeof(NF_Runtime_ISR_MemoryOffsetType);
    NF_Runtime_ISR_MemoryOffsetType *indexToReadData = (NF_Runtime_ISR_MemoryOffsetType *)dataBuffer;
    dataBuffer += sizeof(NF_Runtime_ISR_MemoryOffsetType);

    NF_Runtime_ISR_MemoryOffsetType nextIndexToWrite = (1 + *indexToWriteData) % capacity;
...
#endif
    CLR_UINT8 *
    ReadUnifiedMemoryPointer(CLR_UINT8 **bytecodePtr, ServiceRoutineMemory &memory)
{
    NF_Runtime_ISR_UMA_Type address = *(NF_Runtime_ISR_UMA_Type *)(*bytecodePtr);
    *bytecodePtr += sizeof(NF_Runtime_ISR_UMA_Type);
    NF_Runtime_ISR_UMA_Type offset = address & NF_RUNTIME_ISR_UMA_OFFSET_MASK;

    CLR_UINT8 *block;
    switch (address & NF_RUNTIME_ISR_UMA_MEMORYTYPE_MASK)
    {
        case NF_RUNTIME_ISR_UMA_MEMORYTYPE_HEAP:
            block = memory.Heap;
            break;
        case NF_RUNTIME_ISR_UMA_MEMORYTYPE_ISR:
            block = memory.OnInterruptMemory;
            break;
        case NF_RUNTIME_ISR_UMA_MEMORYTYPE_RTOSTASK:
            block = memory.AfterInterruptMemory;
            break;
        default:
            block = memory.ManagedActivationMemory;
            break;
    }

    if ((address & NF_RUNTIME_ISR_UMA_INDIRECT_REFERENCE_FLAG) != 0)
    {
        offset = *(NF_Runtime_ISR_UMA_Type *)(block + offset);
        if ((address & NF_RUNTIME_ISR_UMA_MEMORYTYPE_MASK) == NF_RUNTIME_ISR_UMA_MEMORYTYPE_HEAP)
        {
            switch (offset & NF_RUNTIME_ISR_UMA_MEMORYTYPE_MASK)
            {
                case NF_RUNTIME_ISR_UMA_MEMORYTYPE_HEAP:
                    block = memory.Heap;
                    break;
            ...
            offset &= NF_RUNTIME_ISR_UMA_OFFSET_MASK;
        }
    }
...
void NF_RunTime_ISR_RunServiceRoutine(NF_Runtime_ISR_ManagedActivation *serviceRoutine)
{
    ...
    if (serviceRoutine->ServiceRoutineMemory != nullptr)
    {
        ...
        NF_Runtime_ISR_MemoryOffsetType heapOffset = *(NF_Runtime_ISR_MemoryOffsetType *)memory.ManagedActivationMemory;
        if (heapOffset != 0)
        {
    ...
    CLR_UINT8 *bytecodePtr;
    switch (serviceRoutine->ServiceRoutineOffset & NF_RUNTIME_ISR_UMA_MEMORYTYPE_MASK)
    {
        case NF_RUNTIME_ISR_UMA_MEMORYTYPE_ISR:
            bytecodePtr =
        ...
        case NF_RUNTIME_ISR_UMA_MEMORYTYPE_RTOSTASK:
            bytecodePtr =
                memory.AfterInterruptMemory + (serviceRoutine->ServiceRoutineOffset & NF_RUNTIME_ISR_UMA_OFFSET_MASK);
            break;
        default:
            bytecodePtr = memory.ManagedActivationMemory +
                          (serviceRoutine->ServiceRoutineOffset & NF_RUNTIME_ISR_UMA_OFFSET_MASK);
            break;
    }
...
bool NF_RunTime_ISR_RunServiceRoutine(NF_Runtime_ISR_ServiceRoutine *serviceRoutine)
{
    ...
    memory.AfterInterruptMemory =
        serviceRoutine->CalledFromTask ? serviceRoutine->InterruptHandler->AfterInterruptMemory : nullptr;
    ...
    if (serviceRoutine->CalledFromTask)
    {
        ...
        {
            NF_Runtime_ISR_MemoryOffsetType heapOffset =
                *(NF_Runtime_ISR_MemoryOffsetType *)memory.AfterInterruptMemory;
            if (heapOffset != 0)
            {
    ...
    else
    {
        ...
        {
            NF_Runtime_ISR_MemoryOffsetType heapOffset = *(NF_Runtime_ISR_MemoryOffsetType *)memory.OnInterruptMemory;
            if (heapOffset != 0)
            {
    ...
    CLR_UINT8 *bytecodePtr;
    if ((serviceRoutine->ServiceRoutineOffset & NF_RUNTIME_ISR_UMA_MEMORYTYPE_MASK) ==
        NF_RUNTIME_ISR_UMA_MEMORYTYPE_RTOSTASK)
    {
        bytecodePtr =
            memory.AfterInterruptMemory + (serviceRoutine->ServiceRoutineOffset & NF_RUNTIME_ISR_UMA_OFFSET_MASK);
    }
...
#endif
{
    ...
    CLR_UINT8 *serviceRoutinePtr = bytecodePtr;
#endif
...
#pragma endregion
        ...
        else if (opCode >= NF_Runtime_ISR_CompiledOpCode::DataBufferGetCapacity)
        {
...
#define INTEGER_MATH_BITWISE_OPERATOR(op, type) *(type *)toLocation op *(type *)fromLocation
                ...
                switch (opCode)
                {
                    case NF_Runtime_ISR_CompiledOpCode::AddInteger8:
                        INTEGER_MATH_BITWISE_OPERATOR(+=, CLR_INT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::AddInteger16:
                        INTEGER_MATH_BITWISE_OPERATOR(+=, CLR_INT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::AddInteger32:
                        INTEGER_MATH_BITWISE_OPERATOR(+=, CLR_INT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::AddInteger64:
                        INTEGER_MATH_BITWISE_OPERATOR(+=, CLR_INT64);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::SubtractInteger8:
                        INTEGER_MATH_BITWISE_OPERATOR(-=, CLR_INT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::SubtractInteger16:
                        INTEGER_MATH_BITWISE_OPERATOR(-=, CLR_INT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::SubtractInteger32:
                        INTEGER_MATH_BITWISE_OPERATOR(-=, CLR_INT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::SubtractInteger64:
                        INTEGER_MATH_BITWISE_OPERATOR(-=, CLR_INT64);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::MultiplyInteger8:
                        INTEGER_MATH_BITWISE_OPERATOR(*=, CLR_INT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::MultiplyInteger16:
                        INTEGER_MATH_BITWISE_OPERATOR(*=, CLR_INT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::MultiplyInteger32:
                        INTEGER_MATH_BITWISE_OPERATOR(*=, CLR_INT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::MultiplyInteger64:
                        INTEGER_MATH_BITWISE_OPERATOR(*=, CLR_INT64);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::DivideInteger8:
                        INTEGER_MATH_BITWISE_OPERATOR(/=, CLR_INT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::DivideInteger16:
                        INTEGER_MATH_BITWISE_OPERATOR(/=, CLR_INT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::DivideInteger32:
                        INTEGER_MATH_BITWISE_OPERATOR(/=, CLR_INT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::DivideInteger64:
                        INTEGER_MATH_BITWISE_OPERATOR(/=, CLR_INT64);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::RemainderInteger8:
                        INTEGER_MATH_BITWISE_OPERATOR(%=, CLR_INT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::RemainderInteger16:
                        INTEGER_MATH_BITWISE_OPERATOR(%=, CLR_INT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::RemainderInteger32:
                        INTEGER_MATH_BITWISE_OPERATOR(%=, CLR_INT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::RemainderInteger64:
                        INTEGER_MATH_BITWISE_OPERATOR(%=, CLR_INT64);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::AndInteger8:
                        INTEGER_MATH_BITWISE_OPERATOR(&=, CLR_UINT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::AndInteger16:
                        INTEGER_MATH_BITWISE_OPERATOR(&=, CLR_UINT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::AndInteger32:
                        INTEGER_MATH_BITWISE_OPERATOR(&=, CLR_UINT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::AndInteger64:
                        INTEGER_MATH_BITWISE_OPERATOR(&=, CLR_UINT64);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::OrInteger8:
                        INTEGER_MATH_BITWISE_OPERATOR(|=, CLR_UINT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::OrInteger16:
                        INTEGER_MATH_BITWISE_OPERATOR(|=, CLR_UINT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::OrInteger32:
                        INTEGER_MATH_BITWISE_OPERATOR(|=, CLR_UINT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::OrInteger64:
                        INTEGER_MATH_BITWISE_OPERATOR(|=, CLR_UINT64);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::XorInteger8:
                        INTEGER_MATH_BITWISE_OPERATOR(^=, CLR_UINT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::XorInteger16:
                        INTEGER_MATH_BITWISE_OPERATOR(^=, CLR_UINT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::XorInteger32:
                        INTEGER_MATH_BITWISE_OPERATOR(^=, CLR_UINT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::XorInteger64:
                        INTEGER_MATH_BITWISE_OPERATOR(^=, CLR_UINT64);
                        break;
                }
            }
...
#define INTEGER_SHIFT_OPERATOR(op, type) *(type *)toLocation op *fromLocation
                ...
                switch (opCode)
                {
                    case NF_Runtime_ISR_CompiledOpCode::ShiftLeftInteger8:
                        INTEGER_SHIFT_OPERATOR(<<=, CLR_UINT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::ShiftLeftInteger16:
                        INTEGER_SHIFT_OPERATOR(<<=, CLR_UINT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::ShiftLeftInteger32:
                        INTEGER_SHIFT_OPERATOR(<<=, CLR_UINT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::ShiftLeftInteger64:
                        INTEGER_SHIFT_OPERATOR(<<=, CLR_UINT64);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::LogicalShiftRightInteger8:
                        INTEGER_SHIFT_OPERATOR(>>=, CLR_UINT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::LogicalShiftRightInteger16:
                        INTEGER_SHIFT_OPERATOR(>>=, CLR_UINT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::LogicalShiftRightInteger32:
                        INTEGER_SHIFT_OPERATOR(>>=, CLR_UINT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::LogicalShiftRightInteger64:
                        INTEGER_SHIFT_OPERATOR(>>=, CLR_UINT64);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::ArithmeticShiftRightInteger8:
                        INTEGER_SHIFT_OPERATOR(>>=, CLR_INT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::ArithmeticShiftRightInteger16:
                        INTEGER_SHIFT_OPERATOR(>>=, CLR_INT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::ArithmeticShiftRightInteger32:
                        INTEGER_SHIFT_OPERATOR(>>=, CLR_INT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::ArithmeticShiftRightInteger64:
                        INTEGER_SHIFT_OPERATOR(>>=, CLR_INT64);
                        break;
                }
            }
...
#define INTEGER_UNARY_OPERATOR2(op, type)                                                                              \
    {                                                                                                                  \
        type x = *(type *)toLocation;                                                                                  \
        op x;                                                                                                          \
        *(type *)toLocation = x;                                                                                       \
    }
                ...
                switch (opCode)
                {
                    case NF_Runtime_ISR_CompiledOpCode::ComplementInteger8:
                        INTEGER_UNARY_OPERATOR(~, CLR_UINT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::ComplementInteger16:
                        INTEGER_UNARY_OPERATOR(~, CLR_UINT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::ComplementInteger32:
                        INTEGER_UNARY_OPERATOR(~, CLR_UINT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::ComplementInteger64:
                        INTEGER_UNARY_OPERATOR(~, CLR_UINT64);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::UnaryMinusInteger8:
                        INTEGER_UNARY_OPERATOR(-, CLR_INT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::UnaryMinusInteger16:
                        INTEGER_UNARY_OPERATOR(-, CLR_INT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::UnaryMinusInteger32:
                        INTEGER_UNARY_OPERATOR(-, CLR_INT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::UnaryMinusInteger64:
                        INTEGER_UNARY_OPERATOR(-, CLR_INT64);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::UnaryMinusMinusInteger8:
                        INTEGER_UNARY_OPERATOR2(--, CLR_INT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::UnaryMinusMinusInteger16:
                        INTEGER_UNARY_OPERATOR2(--, CLR_INT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::UnaryMinusMinusInteger32:
                        INTEGER_UNARY_OPERATOR2(--, CLR_INT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::UnaryMinusMinusInteger64:
                        INTEGER_UNARY_OPERATOR2(--, CLR_INT64);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::UnaryPlusPlusInteger8:
                        INTEGER_UNARY_OPERATOR2(++, CLR_UINT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::UnaryPlusPlusInteger16:
                        INTEGER_UNARY_OPERATOR2(++, CLR_UINT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::UnaryPlusPlusInteger32:
                        INTEGER_UNARY_OPERATOR2(++, CLR_UINT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::UnaryPlusPlusInteger64:
                        INTEGER_UNARY_OPERATOR2(++, CLR_UINT64);
                        break;
                }
            }
...
#define COMPARISON_OPERATOR(op, type)                                                                                  \
    *(CLR_UINT8 *)toLocation = (*(type *)value1Location op * (type *)value2Location) ? 0xFF : 0
                ...
                switch (opCode)
                {
                    case NF_Runtime_ISR_CompiledOpCode::AreEqualInteger8:
                        COMPARISON_OPERATOR(==, CLR_UINT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::AreEqualInteger16:
                        COMPARISON_OPERATOR(==, CLR_UINT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::AreEqualInteger32:
                        COMPARISON_OPERATOR(==, CLR_UINT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::AreEqualInteger64:
                        COMPARISON_OPERATOR(==, CLR_UINT64);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::AreNotEqualInteger8:
                        COMPARISON_OPERATOR(!=, CLR_UINT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::AreNotEqualInteger16:
                        COMPARISON_OPERATOR(!=, CLR_UINT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::AreNotEqualInteger32:
                        COMPARISON_OPERATOR(!=, CLR_UINT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::AreNotEqualInteger64:
                        COMPARISON_OPERATOR(!=, CLR_UINT64);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsLessOrEqualSignedInteger8:
                        COMPARISON_OPERATOR(<=, CLR_INT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsLessOrEqualSignedInteger16:
                        COMPARISON_OPERATOR(<=, CLR_INT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsLessOrEqualSignedInteger32:
                        COMPARISON_OPERATOR(<=, CLR_INT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsLessOrEqualSignedInteger64:
                        COMPARISON_OPERATOR(<=, CLR_INT64);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsLessOrEqualUnsignedInteger8:
                        COMPARISON_OPERATOR(<=, CLR_UINT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsLessOrEqualUnsignedInteger16:
                        COMPARISON_OPERATOR(<=, CLR_UINT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsLessOrEqualUnsignedInteger32:
                        COMPARISON_OPERATOR(<=, CLR_UINT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsLessOrEqualUnsignedInteger64:
                        COMPARISON_OPERATOR(<=, CLR_UINT64);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsLessThanSignedInteger8:
                        COMPARISON_OPERATOR(<, CLR_INT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsLessThanSignedInteger16:
                        COMPARISON_OPERATOR(<, CLR_INT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsLessThanSignedInteger32:
                        COMPARISON_OPERATOR(<, CLR_INT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsLessThanSignedInteger64:
                        COMPARISON_OPERATOR(<, CLR_INT64);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsLessThanUnsignedInteger8:
                        COMPARISON_OPERATOR(<, CLR_UINT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsLessThanUnsignedInteger16:
                        COMPARISON_OPERATOR(<, CLR_UINT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsLessThanUnsignedInteger32:
                        COMPARISON_OPERATOR(<, CLR_UINT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsLessThanUnsignedInteger64:
                        COMPARISON_OPERATOR(<, CLR_UINT64);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsGreaterOrEqualSignedInteger8:
                        COMPARISON_OPERATOR(>=, CLR_INT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsGreaterOrEqualSignedInteger16:
                        COMPARISON_OPERATOR(>=, CLR_INT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsGreaterOrEqualSignedInteger32:
                        COMPARISON_OPERATOR(>=, CLR_INT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsGreaterOrEqualSignedInteger64:
                        COMPARISON_OPERATOR(>=, CLR_INT64);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsGreaterOrEqualUnsignedInteger8:
                        COMPARISON_OPERATOR(>=, CLR_UINT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsGreaterOrEqualUnsignedInteger16:
                        COMPARISON_OPERATOR(>=, CLR_UINT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsGreaterOrEqualUnsignedInteger32:
                        COMPARISON_OPERATOR(>=, CLR_UINT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsGreaterOrEqualUnsignedInteger64:
                        COMPARISON_OPERATOR(>=, CLR_UINT64);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsGreaterThanSignedInteger8:
                        COMPARISON_OPERATOR(>, CLR_INT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsGreaterThanSignedInteger16:
                        COMPARISON_OPERATOR(>, CLR_INT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsGreaterThanSignedInteger32:
                        COMPARISON_OPERATOR(>, CLR_INT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsGreaterThanSignedInteger64:
                        COMPARISON_OPERATOR(>, CLR_INT64);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsGreaterThanUnsignedInteger8:
                        COMPARISON_OPERATOR(>, CLR_UINT8);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsGreaterThanUnsignedInteger16:
                        COMPARISON_OPERATOR(>, CLR_UINT16);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsGreaterThanUnsignedInteger32:
                        COMPARISON_OPERATOR(>, CLR_UINT32);
                        break;
                    case NF_Runtime_ISR_CompiledOpCode::IsGreaterThanUnsignedInteger64:
                        COMPARISON_OPERATOR(>, CLR_UINT64);
                        break;
                }
            }
...
#pragma region Memory operations
                ...
                {
                    CLR_UINT8 *toLocation = ReadUnifiedMemoryPointer(&bytecodePtr, memory);
                    NF_Runtime_ISR_MemoryOffsetType numBytes = *(NF_Runtime_ISR_MemoryOffsetType *)bytecodePtr;
                    bytecodePtr += sizeof(NF_Runtime_ISR_MemoryOffsetType);
                ...
                case NF_Runtime_ISR_CompiledOpCode::CopyTypecastData:
                {
                    ...
                    // We only need to have small signed => large signed and small unsigned => large unsigned
                    // conversions. For same sized integers, the SourceCodeCompiler uses CopyData. For large integer =>
                    // small integer, the SourceCodeCompiler uses CopyData. For small signed => large  integer, the
                    // SourceCodeCompiler uses signed => signed. For small unsigned => large signed, the
                    // SourceCodeCompiler uses unsigned => unsigned.
                    ...
                    {
                        case (
                            (int)NF_Runtime_ISR_CompiledDataType::UInt16 +
                            (((int)NF_Runtime_ISR_CompiledDataType::UInt8) << 4)):
                            *(CLR_UINT16 *)toLocation = (CLR_UINT16)(*(CLR_UINT8 *)fromLocation);
                            break;
                        case (
                            (int)NF_Runtime_ISR_CompiledDataType::UInt32 +
                            (((int)NF_Runtime_ISR_CompiledDataType::UInt8) << 4)):
                            *(CLR_UINT32 *)toLocation = (CLR_UINT32)(*(CLR_UINT8 *)fromLocation);
                            break;
                        case (
                            (int)NF_Runtime_ISR_CompiledDataType::UInt32 +
                            (((int)NF_Runtime_ISR_CompiledDataType::UInt16) << 4)):
                            *(CLR_UINT32 *)toLocation = (CLR_UINT32)(*(CLR_UINT16 *)fromLocation);
                            break;
                        case (
                            (int)NF_Runtime_ISR_CompiledDataType::UInt64 +
                            (((int)NF_Runtime_ISR_CompiledDataType::UInt8) << 4)):
                        ...
                        case (
                            (int)NF_Runtime_ISR_CompiledDataType::UInt64 +
                            (((int)NF_Runtime_ISR_CompiledDataType::UInt32) << 4)):
                            *(CLR_UINT64 *)toLocation = (CLR_UINT64)(*(CLR_UINT32 *)fromLocation);
                            break;

                        case (
                            (int)NF_Runtime_ISR_CompiledDataType::Int16 +
                            (((int)NF_Runtime_ISR_CompiledDataType::Int8) << 4)):
                            *(CLR_INT16 *)toLocation = (CLR_INT16)(*(CLR_INT8 *)fromLocation);
                            break;
                        case (
                            (int)NF_Runtime_ISR_CompiledDataType::Int32 +
                            (((int)NF_Runtime_ISR_CompiledDataType::Int8) << 4)):
                            *(CLR_INT32 *)toLocation = (CLR_INT32)(*(CLR_INT8 *)fromLocation);
                            break;
                        case (
                            (int)NF_Runtime_ISR_CompiledDataType::Int32 +
                            (((int)NF_Runtime_ISR_CompiledDataType::Int16) << 4)):
                            *(CLR_INT32 *)toLocation = (CLR_INT32)(*(CLR_INT16 *)fromLocation);
                            break;
                        case (
                            (int)NF_Runtime_ISR_CompiledDataType::Int64 +
                            (((int)NF_Runtime_ISR_CompiledDataType::Int8) << 4)):
                        ...
                        case (
                            (int)NF_Runtime_ISR_CompiledDataType::Int64 +
                            (((int)NF_Runtime_ISR_CompiledDataType::Int32) << 4)):
                ...
                case NF_Runtime_ISR_CompiledOpCode::UpdateMemoryOffset:
                {
                    CLR_UINT8 *toLocation = ReadUnifiedMemoryPointer(&bytecodePtr, memory);
                    CLR_UINT8 *fromLocation = ReadUnifiedMemoryPointer(&bytecodePtr, memory);
                    NF_Runtime_ISR_MemoryOffsetType elementSize = *(NF_Runtime_ISR_MemoryOffsetType *)bytecodePtr;
                    bytecodePtr += sizeof(NF_Runtime_ISR_MemoryOffsetType);

                    *(NF_Runtime_ISR_MemoryOffsetType *)toLocation +=
                        elementSize * *(NF_Runtime_ISR_MemoryOffsetType *)fromLocation;
                    break;
                }
#pragma endregion
            }
