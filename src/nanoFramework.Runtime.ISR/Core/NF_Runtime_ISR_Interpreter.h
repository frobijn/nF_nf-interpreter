...
//----------------------------------------------------------------------
//
// Interfaces to native components: interrupt generators, data buses.
//
//----------------------------------------------------------------------
...
{
    /// <summary>
    /// Execute a method of the data bus. The number of arguments and the possibility of returning a value
    /// are derived from the method as specified in the assembly used in the compilation of the *.isr.cs file
    /// (typically in the <c>nanoFramework.Runtime.ISR.Specification</c> namespace).
    /// </summary>
    /// <param name="dataBus">A pointer to this structure.</param>
    /// <param name="methodIndex">The index of the method to execute.</param>
    /// <param name="dataCount">The number of elements in the <paramref name="dataPtr"/> and <paramref name="dataSize"/>
    /// arrays.</param>
    /// <param name="dataPtr">An array of pointers to the memory that holds the method's arguments. If
    /// the method does not require any arguments <see langword="nullptr" /> is passed.</param>
    /// <param name="dataSize">An array with the total size of the data pointed to by the corresponding
    /// <paramref name="dataPtr"/>. If the method does not require any arguments <see langword="nullptr" />
    /// is passed.</param>
    /// <param name="result">If the method of the data bus specification has declared a return type,
    /// and that return type is used in the code of the service routine: a pointer to the (uninitialized)
    /// memory to write the result to. Otherwise <see langword="nullptr" /> is passed.
    /// </param>
    /// <param name="resultSize">The total size of the data pointed to by <paramref name="result"/>.
    /// If the method is not required to return a result 0 is passed.</param>
    void (*Execute)(
...
/// </summary>
typedef struct __nfpack NF_Runtime_ISR_Configuration
{
    /// <summary>
    /// The unique ID of the service manager, required to raise a managed event.
    /// </summary>
    CLR_UINT32 ServiceManagerId;
    /// <summary>
    /// Indicates whether ISR memory should be treated as special, to select only
    /// the memory that is available to native interrupt handlers.
    /// </summary>
    bool IsISRMemorySpecial;
    /// <summary>
    /// The size of the queue for triggering running of "after interrupt" service routines.
    /// </summary>
    CLR_UINT16 InterruptQueueSize;
    /// <summary>
    /// The stack size for the task that runs the "after interrupt" service routines.
    /// If this is 0 set it to the system default, the actual value that is used.
    /// </summary>
    CLR_UINT32 AfterInterruptStackSize;
    /// <summary>
    /// The priority of the task that runs the "after interrupt" service routines.
    /// If this is 0 set it to the system default, the actual value that is used.
    /// </summary>
    CLR_UINT32 AfterInterruptPriority;
    /// <summary>
    /// The stack size for the task that runs the service routine passed to
    /// <c>ManagedCode.OnCall</c>.
    /// If this is 0 set it to the system default, the actual value that is used.
    /// </summary>
    CLR_UINT32 ManagedTaskStackSize;
    /// <summary>
    /// The priority of the task that runs the service routine passed to
    /// <c>ManagedCode.OnCall</c>.
    /// If this is 0 set it to the system default, the actual value that is used.
    /// </summary>
    CLR_UINT32 ManagedTaskPriority;
    /// <summary>
    /// Indicates whether the use of resources should be monitored
    /// </summary>
    bool MonitorResourceUse;
    /// <summary>
    /// The required size of the queue for triggering running of "after interrupt" service routines.
    /// </summary>
    CLR_UINT16 RequiredInterruptQueueSize;
    /// <summary>
    /// The required size of the stack for the task that runs the "after interrupt" service routines.
    /// </summary>
    CLR_UINT32 RequiredAfterInterruptStackSize;
    /// <summary>
    /// The required size of the stack for the task that runs the service routine passed to
    /// <c>ManagedCode.OnCall</c>.
    /// </summary>
    CLR_UINT32 RequiredManagedTaskStackSize;
} NF_Runtime_ISR_Configuration;

#define HAS_CONFIGURATION
#define CONFIGURATION_ARGUMENT       NF_Runtime_ISR_Configuration *configuration
#define CONFIGURATION_ARGUMENT_COMMA NF_Runtime_ISR_Configuration *configuration,
#else
#define CONFIGURATION_ARGUMENT       /**/
#define CONFIGURATION_ARGUMENT_COMMA /**/
#endif
...
{
    /// <summary>Initialise this value with the <c>OnInterruptHandlers._onInterruptMemory</c> property
    /// value.</summary>
    ...
    /// <summary>Initialise this value with the <c>OnInterruptHandlers._onInterruptOffset</c> property
    /// value.</summary>
    ...
    /// <summary>Initialise this value with the <c>OnInterruptHandlers._afterInterruptSetArgumentsOffset</c>
...
#ifndef NF_RUNTIME_ISR_UNITTESTS
    /// <summary>Initialise this value with the <c>OnInterruptHandlers._configuration</c> property value.</summary>
...
typedef struct __nfpack NF_Runtime_ISR_ServiceRoutine
{
    ...
    bool CalledFromTask;
    /// <summary>Pass the <c>NF_Runtime_ISR_InterruptHandler.*Offset</c> corresponding to the service
    /// routine.</summary>
...
{
    /// <summary>Initialise this value with the <c>OnManagedActivation._onInterruptMemory</c> property
    /// value.</summary>
    CLR_UINT8 *OnInterruptMemory;
    /// <summary>Initialise this value with the <c>OnManagedActivation._afterInterruptMemory</c> property
    /// value.</summary>
    CLR_UINT8 *AfterInterruptMemory;
    /// <summary>Initialise this value with the <c>OnManagedActivation._activationMemory</c> property
    /// value.</summary>
    ...
    NF_Runtime_ISR_MemoryOffsetType ServiceRoutineOffset;
#ifndef NF_RUNTIME_ISR_UNITTESTS
    /// <summary>Initialise this value with the <c>OnManagedActivation._configuration</c> property value.</summary>
    NF_Runtime_ISR_Configuration *Configuration;
#endif
    /// <summary>Method to raise an event that will be handled in managed code.</summary>
    NF_Runtime_ISR_RaiseManagedEvent RaiseManagedEvent;
...
#ifndef NF_RUNTIME_ISR_UNITTESTS
/// <summary>
/// Method to initialise the struct from an argument of type <c>nanoFramework.Runtime.ISR.ManagedActivation</c> that
/// is passed by managed code.
/// </summary>
...
/// reserved.</param>
extern CLR_UINT8 *NF_Runtime_ISR_DataBuffer_GetMemory(CLR_UINT8 *memory, NF_Runtime_ISR_MemoryOffsetType offsetOffset);
...
/// A null pointer is returned if the buffer is at capacity or if <paramref name="data"/>
/// is not a null pointer.</returns>
extern CLR_UINT8 *NF_Runtime_ISR_DataBuffer_Set(
    CLR_UINT8 *dataBuffer,
...
/// to capacity.</param>
/// <returns>Pointer to the memory where the data has to be copied to.
/// A null pointer is returned if the buffer is at capacity and <paramref name="allowOverwrite"/> is
/// <see langword="false"/>, or if <paramref name="data"/> is not a null pointer.</returns>
...
/// the buffer.</param>
/// <returns>Pointer to the memory where the data has to be copied from.
/// A null pointer is returned if <paramref name="data"/> is not a null pointer or <paramref name="data"/> is
/// false.</returns>
